...
@dataclass
class MinerUserEmbedderConfig:
    model_path: PathLike
    device: str = "cpu"
    max_clicks_per_user: int = 50


class MinerUserEmbedder(Component):
    config: MinerUserEmbedderConfig

    def __init__(self, config: MinerUserEmbedderConfig | None = None, **kwargs):
        super().__init__(config, **kwargs)

        #need to load Miner model here in the init, assign it to self.user_encoder
        model_cfg = ModelConfig()
        checkpoint = load_file(self.config.model_path)
        #make a minerEncoder object instead of a userEncoder
    ...
    def build_user_embedding(self, click_history: list[Click], article_embeddings):
        ...
        #dictionary from article ids to article embeddings
        ...
        #call miner model, make sure what we are passing is right size and shape and parameteres are in the right order.
