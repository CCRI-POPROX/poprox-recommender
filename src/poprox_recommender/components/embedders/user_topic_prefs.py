...
TOPIC_ARTICLES = []
TOPIC_EMBEDDINGS = {}
with safe_open(model_file_path("topic_embeddings.safetensors"), framework="pt", device="cpu") as f:
...
class UserOnboardingEmbedder(NRMSUserEmbedder):
    ...
    def __call__(
        ...
        if self.config.embedding_source == "static":
            topic_embeddings_by_uuid = TOPIC_EMBEDDINGS
        elif self.config.embedding_source == "candidates":
            topic_embeddings_by_uuid = {**TOPIC_EMBEDDINGS, **embeddings_from_candidates}
            ...
            all_topic_uuids = set(TOPIC_EMBEDDINGS) | set(embeddings_from_candidates) | set(embeddings_from_clicked)
            topic_embeddings_by_uuid = {}
            for topic_uuid in all_topic_uuids:
                def_emb = TOPIC_EMBEDDINGS.get(topic_uuid, th.zeros(768, device=self.config.device))
