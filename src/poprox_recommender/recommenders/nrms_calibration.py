...
from poprox_recommender.components.diversifiers import LocalityCalibrator, TopicCalibrator
...
def topic_calibration_pipeline(num_slots: int, device: str) -> Pipeline:
    """
    Create a recommendation pipeline that uses NRMS for scoring and
    topic calibration for ranking
    ...
    o_rank = pipeline.add_component("reranker", calibrator, candidate_articles=o_scored, interest_profile=e_user)
...
def locality_calibration_pipeline(num_slots: int, device: str) -> Pipeline:
    """
    Create a recommendation pipeline that uses NRMS for scoring and
    locality calibration for ranking
