...
from poprox_recommender.components.selectors.top_news import TopGeneralNewsCandidates
...
def configure(builder: PipelineBuilder, num_slots: int, device: str):
    ...
    # Sections
    ts_candidates = builder.add_component(
        "ts_candidates",
        TopGeneralNewsCandidates,
        candidate_articles=fusion,
        article_packages=i_packages,
    )

    ts_topk = builder.add_component("ts_topk", TopkRanker, TopkConfig(num_slots=3), candidate_articles=ts_candidates)

    ts_config = AddSectionConfig(title="Top Stories", personalized=False)
    ts_sections = builder.add_component("top_stories", AddSection, ts_config, new_section=ts_topk)

    # Topical Sections
    topic1_deduped = builder.add_component("topic1_deduped", DuplicateFilter, candidate=fusion, sections=ts_sections)
