...
from poprox_recommender.components.filters.entities import MentionedEntitiesConfig, MentionedEntitiesFilter
...
def configure(builder: PipelineBuilder, num_slots: int, device: str):
    ...
    f_mentioned = builder.add_component(
        ...
        MentionedEntitiesConfig(entities=set(["Sports"]), entity_type="topic"),
        candidates=fusion,
    )
    ...
    yts_candidates = builder.add_component(
        "yts_candidates",
        TopStoryCandidates,
        candidate_articles=f_mentioned,
    ...
    # Topical Sections
    topic1_deduped = builder.add_component(
        "topic1_deduped", DuplicateFilter, candidate=f_mentioned, sections=yts_sections
    )
    ...
    topic2_deduped = builder.add_component(
        "topic2_deduped", DuplicateFilter, candidate=f_mentioned, sections=topic1_sections
    )
    ...
    topic3_deduped = builder.add_component(
        "topic3_deduped", DuplicateFilter, candidate=f_mentioned, sections=topic2_sections
    )
    ...
    ion_deduped = builder.add_component("ion_deduped", DuplicateFilter, candidate=f_mentioned, sections=topic3_sections)

    ion_filtered = builder.add_component(
        "ion_topic_filtered", TopicPrefsFilter, candidates=ion_deduped, interest_profile=i_profile
    )
    ...
    ion_topk_unfiltered = builder.add_component(
        "ion_topk_unfiltered", TopkRanker, TopkConfig(num_slots=6), candidate_articles=ion_deduped
    )
