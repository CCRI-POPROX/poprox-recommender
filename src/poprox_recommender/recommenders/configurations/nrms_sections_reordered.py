...
from poprox_recommender.components.sections.combine import AddSectionConfig, AppendSection, PrependSection
...
def configure(builder: PipelineBuilder, num_slots: int, device: str):
    ...
    # Top Stories
    yts_deduped = builder.add_component("yts_deduped", DuplicateFilter, candidate=fusion, sections=topic3_sections)

    yts_candidates = builder.add_component(
        "yts_candidates",
        TopStoryCandidates,
        candidate_articles=yts_deduped,
    ...
    yts_sections = builder.add_component(
        "top_stories",
        PrependSection,
        yts_config,
        new_section=yts_fill,
        existing_sections=topic3_sections,
    )

    # In Other News section
    ion_deduped = builder.add_component("ion_deduped", DuplicateFilter, candidate=fusion, sections=yts_sections)
    ...
    builder.add_component(
        "recommender", AppendSection, ion_config, new_section=ion_fill, existing_sections=yts_sections
    )
