...
from poprox_recommender.components.diversifiers import MMRDiversifier
...
def nrms_mmr_pipeline(num_slots: int, device: str) -> Pipeline:
    """
    Create a recommendation pipeline that uses NRMS for scoring and MMR for ranking
    ...
    o_rank = pipeline.add_component("reranker", mmr, candidate_articles=o_scored, interest_profile=e_user)
