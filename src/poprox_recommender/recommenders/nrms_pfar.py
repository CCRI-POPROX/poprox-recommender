...
def nrms_pfar_pipeline(num_slots: int, device: str) -> Pipeline:
    """
    Create a recommendation pipeline that uses NRMS for scoring and PFAR for ranking
    ...
    o_rank = pipeline.add_component("reranker", pfar, candidate_articles=o_scored, interest_profile=e_user)
