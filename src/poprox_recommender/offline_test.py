...
def recsys_metric(recommendations, row_index, news_struuid_ID ):
    ...
    impressions_truth = pd.read_table(f"{src_dir()}/data/test_mind_large/behaviors.tsv",
                                    header='infer',
                                    usecols=range(5),
                                    names=[
...
if __name__ == '__main__':
    ...
    with open(f"{src_dir()}/data/val_mind_large/news_uuid_ID.json", 'r') as json_file:
        news_struuid_ID = json.load(json_file)

    # load the mind test json file
    with open(f"{src_dir()}/data/val_mind_large/mind_test.json", 'r') as json_file:
