...
def compute_rec_metric(recs_df: pd.DataFrame, request: RecommendationRequest):
    ...
    recs = recs_df[recs_df["user"] == request.interest_profile.profile_id]
    ...
    ranked = recs[recs["stage"] == "ranked"]
    reranked = recs[recs["stage"] == "reranked"]

    if not ranked.empty and not reranked.empty:
