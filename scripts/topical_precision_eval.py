...
def store_rec_article_as_csv(
    ...
    csv_rows.append(("Article_id", "Headline", persona_topic, "Relevance", "Source"))
...
def daily_persona_wise_eval(candidate_articles, persona_topic, response):
    candidate_count = 0
    cand_articles_list = []
    seen_headlines = set()
    for article in candidate_articles:
        ...
        if persona_topic in article_topics and article_headline not in seen_headlines:
            candidate_count += 1
            score, src = next(
                ((m.relevance, m.source) for m in article.mentions if m.entity and m.entity.name == persona_topic),
                (0.0, None),
            )
    ...
    return precision, topical_count, candidate_count, rec_articles_list, cand_articles_list
    ...
    for row in day_df.itertuples():
        ...
        for entity in article_topics:
            if entity in TOPIC_TO_UUID.keys():
                ...
                if len(balanced_by_topic[entity]) < 10:
...
for persona_topic, persona_profile in synthetic_personas.items():
    ...
    # calculating the daily precision for each persona
