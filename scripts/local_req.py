...
if __name__ == "__main__":
    ...
    event_feedback_score = {
        "body": raw_json,
        "queryStringParameters": {"pipeline": "nrms_feedback_scores"},
        "isBase64Encoded": False,
    }
    event_fm_score = {
        "body": raw_json,
    ...
    response_fm_score = root(req.model_dump(), pipeline="fm_nrms")
    response_fm_score = RecommendationResponseV2.model_validate(response_fm_score)
    ...
    print("\n")
    print(f"{event_fm_score['queryStringParameters']['pipeline']}")

    for idx, article in enumerate(response_fm_score.recommendations.articles):
        article_topics = extract_general_topics(article)
        print(f"{idx + 1}. {article.headline} {article_topics}")
