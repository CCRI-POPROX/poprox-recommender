...
if __name__ == "__main__":
    with open(project_root() / "tests/request_data/karl-onb-request-2.json", "r") as req_file:
        raw_json = req_file.read()
        req = RecommendationRequestV2.model_validate_json(raw_json)

    event_nrms = {
        "body": raw_json,
        "queryStringParameters": {"pipeline": "nrms_images"},
    ...
    response_nrms = root(req.model_dump(), pipeline="nrms_images")
