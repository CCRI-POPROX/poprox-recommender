...
def test_masking():
    ...
    unmasked_additive, unmasked_additive_outputs, unmasked_additive_weights = news_encoder.additive_attention(
        masked_mha_output
    )
    masked_additive, masked_additive_outputs, masked_additive_weights = news_encoder.additive_attention(
        masked_mha_output, padding_mask=padding_mask
    )

    # Check that masking made a difference
    assert not th.equal(unmasked_additive_weights, masked_additive_weights)
    assert not th.equal(unmasked_additive_outputs, masked_additive_outputs)
