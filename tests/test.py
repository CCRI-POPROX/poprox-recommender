...
class NewsDataset(Dataset):
    ...
    def __init__(self, args, news_path):
        ...
        self.news_parsed = pd.read_table(
            news_path,
            usecols=['id'] + [self.args.dataset_attributes],
            converters={
                attribute: literal_eval
                for attribute in [self.args.dataset_attributes]
            })
...
@torch.no_grad()
def evaluate(args, model, device, directory):
    ...
    # convert all news into vectors
    news2vector = {}
    for minibatch in tqdm(news_dataloader,
                          desc="Calculating vectors for news"):
        news_ids = minibatch["id"]
        if any(id not in news2vector for id in news_ids):
            news_vector = model.get_news_vector(minibatch['title'].to(device))
    ...
    # convert all users to interest vectors
    user2vector = {}
...
if __name__ == '__main__':
    ...
    directory = '../data/test_mind_large'

    auc, mrr, ndcg5, ndcg10 = evaluate(args, model, device, directory)
    print(
        f'AUC: {auc:.4f}\nMRR: {mrr:.4f}\nnDCG@5: {ndcg5:.4f}\nnDCG@10: {ndcg10:.4f}'
    )